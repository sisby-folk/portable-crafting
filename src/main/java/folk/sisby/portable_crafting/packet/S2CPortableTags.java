...
public record S2CPortableTags(List<Item> items, List<TagKey<Item>> tags) implements CustomPayload {
	public static final CustomPayload.Id<S2CPortableTags> ID = new CustomPayload.Id<>(PortableCrafting.id("s2c_portable_tags"));
	public static final PacketCodec<RegistryByteBuf, S2CPortableTags> CODEC = PacketCodec.tuple(
		PacketCodecs.registryValue(RegistryKeys.ITEM).collect(PacketCodecs.toList()), S2CPortableTags::items,
